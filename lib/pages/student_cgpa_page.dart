...
class _StudentCgpaPageState extends State<StudentCgpaPage> {
  ...
  final List<_CgpaSubjectEntry> _entries = [_CgpaSubjectEntry()];
  ...
  void _calculateFromForm() {
    ...
    if (semName.isEmpty && widget.existingSemesterId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
    ...
    final sgpa = totalCredits > 0 ? totalPoints / totalCredits : 0.0;
      ...
      SnackBar(
        content: Row(
          children: [
            ...
            Text(
              'Semester SGPA: ${sgpa.toStringAsFixed(2)} — tap Save to store',
            ),
          ],
        ),
  ...
  Future<void> _save() async {
    ...
    if (semName.isEmpty && widget.existingSemesterId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please enter semester name before saving'),
          backgroundColor: Color(0xFFEF4444),
        ),
      );
      return;
    ...
    if (widget.existingSemesterId != null) {
      ...
      setState(() {
        for (final e in _entries) e.dispose();
        _entries.clear();
        _entries.add(_CgpaSubjectEntry());
        _calculated = false;
      });
      return;
    }
    ...
    final updatedCgpa = provider.getOrCreateGradeData(widget.student.id).cgpa;
    await _addToHistory(
      type: 'CGPA',
      result: updatedCgpa,
      semesterName: semesterName,
  ...
  void _addEntry() => setState(() => _entries.add(_CgpaSubjectEntry()));
        ...
        builder: (context, provider, _) {
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                _buildCgpaCard(isDark, gradeData.cgpa),
                ...
                if (gradeData.semesters.isNotEmpty) ...[
                  Text(
                    'Saved Semesters (${gradeData.semesters.length})',
                    style: TextStyle(
                      fontSize: 16,
                      fontWeight: FontWeight.bold,
                      color: isDark ? Colors.white : Colors.black87,
                  ...
                  ...gradeData.semesters.asMap().entries.map(
                    (e) => _buildSemesterTile(isDark, e.value, e.key, provider),
                  ),
                  const SizedBox(height: 24),
                ],
                _buildCalculateButton(isDark, gradeData.cgpa),
  ...
  Widget _buildCgpaCard(bool isDark, double cgpa) {
    return Container(
        ...
        child: Row(
          children: [
            ...
            IconButton(
              ...
              onPressed: () =>
                  provider.deleteSemester(widget.student.id, sem.id),
            ),
          ],
        ),
      ),
  ...
  Widget _buildCalculateButton(bool isDark, double cgpa) {
    return SizedBox(
      width: double.infinity,
      height: 58,
      child: DecoratedBox(
